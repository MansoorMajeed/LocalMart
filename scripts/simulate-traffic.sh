...
# Service Configuration for Docker setup
declare -A SERVICES=(
    ["catalog"]="${HOST}:${CATALOG_PORT}"
    # Note: Only catalog service is currently implemented
    # ["cart"]="${HOST}:8081"
    # ["orders"]="${HOST}:8082"
    # ["users"]="${HOST}:8083"
)
...
check_all_services() {
    ...
    if [[ ${#AVAILABLE_SERVICES[@]} -eq 0 ]]; then
        ...
        log $YELLOW "💡 Make sure services are running with Docker Compose:"
        log $YELLOW "💡 Run: docker-compose up -d"
        log $YELLOW "💡 Services should be accessible on localhost at configured ports"
        exit 1
    fi
...
show_usage() {
    ...
    echo "  --host HOST               Host for services (default: localhost)"
    echo "  --catalog-port PORT       Port for catalog service (default: 8080)"
    ...
    echo "  catalog    - Product catalog service (Go) - Currently implemented"
    echo ""
    echo "Future Services (not yet implemented):"
    echo "  cart       - Shopping cart service"
    echo "  orders     - Order management service"
    echo "  users      - User management service"
    echo ""
    echo "Examples:"
    echo "  $0                                    # Test all available services"
    echo "  $0 --services catalog                # Test only catalog service"
    ...
    echo "  $0 --catalog-port 8090               # Use different port for catalog"
    echo ""
    echo "Prerequisites:"
    echo "  Run Docker Compose to start services:"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --host)
            HOST="$2"
            # Update services with new host
            SERVICES["catalog"]="${HOST}:${CATALOG_PORT}"
            shift 2
            ;;
        --catalog-port)
            CATALOG_PORT="$2"
            # Update catalog service with new port
            SERVICES["catalog"]="${HOST}:${CATALOG_PORT}"
            shift 2
            ;;
        --verbose)
